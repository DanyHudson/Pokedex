...
function renderModalHeader(id, name, index) {
    return `
        <div class="modal-header" style="border: none;"> 
            <button class="prevPokemn" type="button" onclick="showPrevPokemon(${index})" style="border-radius: 50%;"><img src="/img/prev.svg"></button> 
            <h5 class="modal-title text-center" style="width: 100%;">No.${id} ${name.charAt(0).toUpperCase() + name.slice(1)}</h5> 
            <button class="nextPokemn" type="button" onclick="showNextPokemon(${index})" style="border-radius: 50%;"><img src="/img/next.svg"></button>
        </div>
    `;
}
